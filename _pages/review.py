...
def page_review():
    """Displays the review page with options to select a module/theme, subfolder, and Markdown file."""
    st.subheader("📖 Knowledge Review :")

    # Path to the root directory of the courses
    root_directory = "_topics"

    # List of subdirectories (modules/themes)
    subdirectories = list_subdirectories(root_directory)
    selected_subdirectory = st.selectbox("Choose a Theme", subdirectories)

    if selected_subdirectory:
        selected_directory = os.path.join(root_directory, selected_subdirectory)

        # List of subdirectories in the selected module/theme
        ...
        st.error("No modules/themes found in the specified directory.")
